...
private:
    ...
    bool parseIED(pugi::xml_node& pSCLNode, char *iedName);
    bool parseAccessPoint(pugi::xml_node& iedNode, const int nIEDID, int &nAPNum, char *strAPNames, bool bAPName);
    bool parseLN0Part(pugi::xml_node& pLDeviceNode, const int nIEDID, const int nLDID);
    bool parseLNPart(pugi::xml_node& pLDeviceNode, const int nIEDID, const int nLDID);
    bool parseDOI(pugi::xml_node& lnNode, const int nIEDID, const int nLDID, const int nLNinstID, const int nLnTypeID);
    void processDAI(pugi::xml_node& daiNode, CDOI& doi, std::string prefix);
    void supplementDAIByDoType(const int nIEDID, const int nLDID, const int nLNinstID, CDOI *pDOI);
    pugi::xml_node findDataTypeNode(const char* szTagName, const char* szTypeId);
    void addMissingDAI(int nDoiId, const char* name);
    bool parseDataSet(pugi::xml_node& ln0Node, const int nIEDID, const int nLDID, const int nLNinstID);
    bool parseReportControl(pugi::xml_node& ln0Node, const int nIEDID, const int nLDID, const int nLNinstID);
    bool parseCommunication(pugi::xml_node& pSCLNode);
    void parseMmsAddress(pugi::xml_node apNode, int subNetId, const char* iedName, const char* apName);
    void parseGseOrSmvConfig(pugi::xml_node commNode, pugi::xml_node apNode, int subNetId, const char* type);
    bool parseDataTypeTemplates(pugi::xml_node& pSCLNode);
    bool parseEnumType(pugi::xml_node& pDataTypeTemplatesElement);
    bool parseDAType(pugi::xml_node& pDataTypeTemplatesElement);
    bool parseDOType(pugi::xml_node& pDataTypeTemplatesElement);
    bool parseLNodeType(pugi::xml_node& pDataTypeTemplatesElement);
private:
    pugi::xml_document *m_pDoc;
    std::string m_currentFile;
    std::string m_currentIed;
    bool m_dataTypeDone;

public:
    std::vector<CLnType> CSCDLnTypeList;                // LNodeType列表
    std::vector<CLnMember> CSCDLnMemberList;            // LNodeType的成员列表
    std::vector<CDoType> CSCDDoTypeList;                // DOType列表
    std::vector<CDoMember> CSCDDoMemberList;            // DOType的成员列表
    std::vector<CDaType> CSCDDaTypeList;                // DAType列表
    std::vector<CDaMember> CSCDDaMemberList;            // DAType的成员列表
    std::vector<CEnumType> CSCDEnumTypeList;            // EnumType列表
    std::vector<CEnumMember> CSCDEnumMemberList;        // EnumType的成员列表
    std::vector<CIED> CSCDIEDList;                      // IED列表
    std::vector<CLD> CSCDLDList;                        // LDevice列表
    std::vector<CLNinst> CSCDLNinstList;                // LN/LN0列表
    std::vector<CDOI> CSCDDOIList;                      // DOI列表
    std::vector<CDAI> CSCDDAIList;                      // CDAI列表
    std::vector<CDataSet> CSCDDataSetList;              // DataSet列表
    std::vector<CDsMember> CSCDDsMemberList;            // FCDA列表
    std::vector<CRptCtrl> CSCDRptCtrlList;              // ReportControl列表
    std::vector<CLLNO> CSCDLLNOList;                    // SettingControl、GSEControl、SampledValueControl列表
    std::vector<CNetConfig> CSCDNetConfigList;          // SubNetwork列表
    std::vector<CCommConfig> CSCDCommConfigList;        // ConnectedAP列表
    std::vector<CHeaderConfig> CSCDHeaderConfigList;    // Header配置列表
    std::vector<CHistoryConfig> CSCDHistoryConfigList;  // History配置列表
