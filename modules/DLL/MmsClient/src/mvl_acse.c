...
ST_VOID mvl_set_max_msg_size (ST_INT max_msg_size)
  ...
  if (mmsl_enc_buf != NULL)
    usr_FREE (MSMEM_STARTUP, mmsl_enc_buf);   // M_FREE

  mmsl_enc_buf = (ST_UCHAR *) usr_MALLOC (MSMEM_STARTUP, mmsl_enc_buf_size);   // M_MALLOC
  }
...
/* OK, now copy the MVL configuration info */
  ...
  mvl_cfg_info = (MVL_CFG_INFO *) usr_calloc (1, mvl_cfg_info_size); //chk_calloc
...
/* Alloc & fill in array of "calling" connection control structures.	*/
  ...
  if (mvl_cfg_info->num_calling != 0)
    ...
    mvl_calling_conn_ctrl = (MVL_NET_INFO *) usr_CALLOC (MSMEM_GEN, mvl_cfg_info->num_calling, sizeof (MVL_NET_INFO)); // M_CALLOC
...
/* Alloc & fill in array of "called" connection control structures.	*/
  ...
  if (mvl_cfg_info->num_called != 0)
    ...
    mvl_called_conn_ctrl = (MVL_NET_INFO *) usr_CALLOC (MSMEM_GEN, mvl_cfg_info->num_called, sizeof (MVL_NET_INFO)); // M_CALLOC
