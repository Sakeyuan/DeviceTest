...
/* --------  ---  ------   -------------------------------------------	*/
/* 2006-10-18  施江涛     SMEM改为只使用MSMEM_GEN,更改M_SMEM_MAX_CONTEXT	*/
...
#ifdef DEBUG_SISCO
  ST_UINT8 maxNumUsed;
  ST_UINT32 *usedSize;		/* usedSize [numBuf] */ // Mod 2006-9-28 从ST_UINT16改为ST_UINT32
#endif
...
ST_RET smemcfgx (ST_CHAR *xml_filename, ST_CHAR *xmlbuf, ST_INT xmlbufLen);
...
#define SMEMLOG_NERR2(a,b,c) {\
                     if (smem_debug_sel & SMEM_LOG_NERR)\
                       _slog (sLogCtrl,_smem_log_nerr_logstr,\
		             thisFileName,__LINE__,a,b,c);\
                    }
#define SMEMLOG_NERR3(a,b,c,d) {\
                     if (smem_debug_sel & SMEM_LOG_NERR)\
                       _slog (sLogCtrl,_smem_log_nerr_logstr,\
...
#define	M_SMEM_MAX_CONTEXT	       1  // 30 /* number of specific context array elements  	       */
