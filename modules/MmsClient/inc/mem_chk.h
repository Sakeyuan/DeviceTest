...
/* --------  ---  ------   -------------------------------------------	*/
/* 2006-10-18  施江涛     SMEM改为只使用MSMEM_GEN,以简化内存配置和使用	*/
...
// 自定义的动态内存分配函数,如果使用的动态内存在程序运行过程中不free,
// 建议使用以下函数，否则使用SISCO提供的chk_xx/M_xx系列函数
  #define usr_MALLOC(ctx,x)     malloc  (x  )
  #define usr_CALLOC(ctx,x,y)   calloc  (x,y)
  #define usr_REALLOC(ctx,x,y)  realloc (x,y)
  #define usr_FREE(ctx,x)       free    (x  )
  #define usr_STRDUP(ctx,x)	    m_strdup(x  )

  #define usr_malloc(x)		    malloc  (x  )
  #define usr_calloc(x,y)	    calloc  (x,y)
  #define usr_realloc(x,y)	    realloc (x,y)
  #define usr_free(x)		    free    (x  )
  #define usr_strdup(x)		    m_strdup(x  )
